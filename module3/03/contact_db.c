...
void writeDB(int fd, ContactDB db) {
    writeNode(db.root, fd);
}

void readDB(int fd, ContactDB *db) {
    ContactData buf;
    while (read(fd, &buf, sizeof(ContactData)) == sizeof(ContactData)) {
        saveContact(db, buf);
    }
}
...
void writeNode(Node *node, int fd) {
    if (!node) {
        return;
    }

    write(fd, &node->data, sizeof(ContactData));
    writeNode(node->right, fd);
    writeNode(node->left, fd);
}
