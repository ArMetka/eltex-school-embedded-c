...
int main(int argc, char *argv[]) {
    ...
    while (!should_exit) {
        int ret = poll(fds, nfds, TIMEOUT_S * 1000);
        if (!ret) {  // timeout
            continue;
        } else if (ret < 0) {  // error
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fds[i].fd == sockfd) {  // accept new connection
                if (nfds == CLIENTS_MAX - 1) {
                    fputs("failed to accept connection: reached client limit", stderr);
                    continue;
                }
                ...
                write(newsockfd, USER_PROMPT_CHOOSE_FUNC, sizeof(USER_PROMPT_CHOOSE_FUNC));
                printf("new connection from %s\n", (char *)inet_ntoa(cli_addr.sin_addr));
                printusers();
            } else {  // handle existing connection
                if (!handle_connection(fds[i].fd)) {
                    nclients--;
                    close(fds[i].fd);
